...
def decrypt(key, blockchain):
    block_size = 16
    key_hash = hashlib.sha256(key).digest()
    plaintext = b''
    
    # Decode the base64 encoded blockchain
    for i in range(0, len(blockchain), block_size):
        cipher_block = blockchain[i:i + block_size]
        block = xor_bytes(cipher_block, key_hash)
        plaintext += block

    # Remove padding
    modified_plaintext = depad(plaintext)

    print("Decrypted Blockchain:", modified_plaintext)
